...
export class PerplexityWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }) {
    ...
    if (thinking && THINKING_MAP[model]) {
      ...
      log?.info?.("PPLX-WEB", `Thinking mode â†’ ${model} using ${modelPref}`);
    ...
    if (!response.ok) {
      ...
      if (status === 401 || status === 403) errMsg = "Perplexity auth failed â€” session cookie may be expired. Re-paste your __Secure-next-auth.session-token.";
