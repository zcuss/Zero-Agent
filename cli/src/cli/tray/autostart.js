...
const APP_LABEL = "com.zero-agent.autostart";
 ...
 *   2. `process.argv[1]` if it's our cli.js — true when Zero Agent is currently
 ...
 * SIGTERM to the running process. When the running Zero Agent cli.js was itself
...
<dict>
    ...
    <string>/tmp/zero-agent.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/zero-agent.error.log</string>
</dict>
</plist>`;
...
function enableWindows(cliPath) {
  ...
  // `Zero Agent.cmd` lookup that depended on the npm prefix path.
