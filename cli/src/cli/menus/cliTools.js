...
async function buildCodexHeader() {
  ...
  const { installed, hasZeroAgent, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasZeroAgent, settings } = result.data;
  ...
  // Extract Zero Agent custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Zero-Agent-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Zero Agent provider config
  const provider = settings?.models?.providers?.["zero-agent"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("zero-agent/") ? primary.replace("zero-agent/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasZeroAgent, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Zero Agent\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
