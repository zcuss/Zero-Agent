...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.zero-agent/runtime
...
// Identifiers for killAllAppProcesses - only kill zero-agent specifically
const PROCESS_IDENTIFIERS = [
  'zero-agent'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "zero-agent")
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running Zero Agent/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "zero-agent" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("zero-agent") && (cmd.includes("cli.js") || cmd.includes("\\zero-agent") || cmd.includes("/zero-agent")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running Zero Agent/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "zero-agent".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("zero-agent") && (cmd.includes("cli.js") || cmd.includes("/zero-agent")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(latestVersion) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Zero Agent is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Zero Agent is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "zero-agent", "db.json") : path.join(".zero-agent", "db.json"));
