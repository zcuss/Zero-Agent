...
import { CODEX_AUTH_MODES } from "./providers/codex/auth";
import { importCodexToken } from "./providers/codex/api";
import { resolveProviderAuthMode } from "./state/providerAuthModes";
import { resolveProviderAuthFlags, resolveProviderInfo } from "./state/providerInfo";
import { useProviderDisabledModels } from "./hooks/useProviderDisabledModels";
import { useProviderAliases } from "./hooks/useProviderAliases";
...
export default function ProviderDetailClient() {
  ...
  const [addApiKeyMode, setAddApiKeyMode] = useState("default");
  ...
  useEffect(() => {
    if (!sessionApiKey && apiKeys.length > 0) setSessionApiKey(apiKeys[0].key);
  }, [apiKeys, sessionApiKey]);

  const openSessionImport = () => {
    setSessionImportError("");
    setSessionImportModalPayloadDefaults();
    setShowSessionImportModal(true);
  };

  const setSessionImportModalPayloadDefaults = () => {
    // keep user's existing payload if already typed
    if (!sessionPayload) {
      setSessionPayload("{\n  \"access_token\": \"\",\n  \"expires\": \"\"\n}");
    }
  };
  ...
  const handleImportSession = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setSessionImportError(data?.error || "Failed to import session");
        return;
      }
      ...
      setSessionImportError(e?.message || "Failed to import session");
    } finally {
  ...
  const {
    ...
    setThinkingMode,
    fetchConnections,
  } = useProviderConnections(providerId, isCompatible);
  ...
  const { authModes, isOAuth, supportsApiKeyAuth, isFreeNoAuth } = resolveProviderAuthFlags(providerId, providerInfo);
  ...
  } = resolveProviderAuthMode({ providerId, isCompatible, isOAuth, supportsApiKeyAuth });
  const codexApiAuthType = CODEX_AUTH_MODES.api;
  const hasSessionPaste = providerId === "codex" || providerId === "openai";
  ...
  } = useProviderDisabledModels(providerStorageAlias);
  const {
    modelAliases,
    setAlias: handleSetAlias,
    deleteAlias: handleDeleteAlias,
  } = useProviderAliases(providerAlias);

  const handleDisableModel = async (modelId) => {
    await disableModels([modelId]);
  };
  ...
  const handleDisableAll = async (ids) => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        await disableModels(ids);
      }
    });
  };

  // Aliases and disabled-model actions are managed by modular hooks
  ...
  // Connections/settings bootstrap handled by `useProviderConnections`
  ...
  // Bootstrap fetches are handled by modular hooks
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
                ...
                <div className="min-w-0">
                  ...
                  {hasDualAuthModes && (
                    <p className="text-xs text-text-muted">
                      Choose OAuth, API token, or paste session access token.
                    </p>
                  )}
                </div>
              </div>
      ...
      <Modal
        ...
        title="Paste Session Access Token"
        onClose={() => {
          if (sessionImportLoading) return;
          setSessionImportError("");
          setShowSessionImportModal(false);
        }}
      >
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs text-text-muted">Session JSON / Access Token Payload</label>
            <textarea
              className="min-h-[240px] w-full rounded-lg border border-border bg-background px-3 py-2 text-xs font-mono text-text-main focus:outline-none focus:ring-1 focus:ring-primary"
              value={sessionPayload}
              onChange={(e) => setSessionPayload(e.target.value)}
              placeholder='{"accessToken":"...","expires":"...","account":{},"user":{}}'
            />
          </div>
