...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* Zero Agent Core - Center */}
      <div className="relative z-20 w-32 h-32 rounded-full bg-[#050b14] border-2 border-cyan-300/70 shadow-[0_0_48px_rgba(34,211,238,0.28)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_32%_20%,rgba(34,211,238,.28),transparent_45%)]" />
        <span className="relative font-black text-5xl leading-none tracking-[-0.16em] text-cyan-100 drop-shadow-[0_0_16px_rgba(34,211,238,.8)] pr-2">
          Z
        </span>
        <span className="relative text-[10px] font-bold text-cyan-100 tracking-widest uppercase">
          Zero Agent
        </span>
        <div className="absolute inset-0 rounded-full border border-cyan-300/35 animate-ping opacity-20"></div>
      </div>
      ...
      {/* SVG Lines from CLI to Zero Agent */}
      <svg
        className="absolute inset-0 w-full h-full z-10 pointer-events-none stroke-cyan-400/55"
      ...
      {/* SVG Lines from Zero Agent to Providers */}
      <svg
