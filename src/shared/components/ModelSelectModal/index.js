...
// Providers that need no auth â€” always show in model selector
...
export default function ModelSelectModal({
  ...
  const groupedModels = useMemo(() => {
    ...
    const filterByKind = (models) => {
      // No kindFilter â†’ LLM context: keep only LLM models (no type or type === "llm")
    ...
    sortedProviderIds.forEach((providerId) => {
      ...
      if (providerInfo.passthroughModels) {
        ...
        // Custom (openai/anthropic-compatible) providers are LLM-only â€” skip for typed media kinds
  ...
  // Filter combos by search query (and hide combos when kindFilter is set â€” combos are LLM-only by design)
